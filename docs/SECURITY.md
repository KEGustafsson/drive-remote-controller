...
| **A stolen or sold phone carries the token onward** | Physical | The token is kept in `EncryptedSharedPreferences` (AES-256-GCM under a hardware-backed master key), the app sets `allowBackup="false"`, and the token is bound to the server that issued it. A token past its stated expiry is dropped at startup. |
...
| (2)(e) Confidentiality of data | **Partial.** Tokens at rest are in `EncryptedSharedPreferences` with cloud backup disabled, and the Android station refuses cleartext off private networks. On the boat's own LAN everything is plain HTTP by construction, and the WiFi and OTA credentials are committed. |
