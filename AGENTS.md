...
```bash
cd esp32 && pio test -e native                          # pure core, all three firmwares (295 cases)
cd esp32 && pio run -e tx_shesp32|rx_shesp32|hh_shesp32 # build
cd esp32 && pio run -e <env> -t upload && pio device monitor
cd sk-plugin && npm test                    # 319 cases
cd sk-plugin && npm run build               # -> public/
cd android && ./gradlew :core:test          # pure Kotlin core, 204 cases
cd android && ./gradlew :app:testDebugUnitTest  # layout + fail-safe UI, 90 cases (needs SDK)
...
- Credentials live in `esp32/include/secrets.h`, which is **gitignored and never
  ...
  `platformio.ini`. Nothing is committed here and nothing ever was; rotation is
  **closed by owner decision** and is not open work -- do not raise it. It would
  reopen only if the predecessor or sibling repository were made public
  (BUILDING.md §8).
